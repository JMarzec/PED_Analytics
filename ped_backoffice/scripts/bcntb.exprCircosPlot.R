...
print(getwd())
    ...
    print(paste0("norm_files/", expFile, "_annotated_hist_",j,".html"))
    htmlwidgets::saveWidget(as_widget(p), paste0("norm_files/", expFile, "_annotated_hist_",j,".html"), selfcontained = FALSE)

    ##### Write the annotated expression data into a file
    write.table(prepare2write(expData.annot), file=paste0("norm_files/", expFile, "_annotated_",j,"csv"), sep="\t", row.names=FALSE)
}
