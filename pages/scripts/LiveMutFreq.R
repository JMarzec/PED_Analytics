...
#
#   Command line use example: Rscript LiveMutFreq.R --mut_file /Users/marzec01/Desktop/git/PED_bioinformatics_portal/PED_Analytics/ped_backoffice/data/genie/norm_files/mut.csv --target /Users/marzec01/Desktop/git/PED_bioinformatics_portal/PED_Analytics/ped_backoffice/data/genie/cn_target.txt --gene KRAS --colouring Target --dir /Users/marzec01/Desktop/git/PED_bioinformatics_portal/PED_Analytics/ped_backoffice/data/genie --hexcode fdshf
#
#   First arg:      Full path with name of the relative linear copy-number matrix
#   Second arg:     Full path with name of the file with mutation data. This file is expected contain the following information: (1) sample name, (2) gene name and (3) variant classification
...
##### Check if the queried genes is present in the mutation data
...
gene.mut.sample <- as.matrix(rep("Not mutated", nrow(annData)))
...
muts <- gene.mut.sample[,"Mutation"]

gene.df <- data.frame(muts, targets)
colnames(gene.df) <- c("Mutation", "Target")


##### Generate horizontal bar plot with bar for each group
p <- gene.df %>% count(Target, Mutation) %>%
plot_ly(x = ~n, y = ~Target, color = ~Mutation, type='bar', orientation = "h", width = 800, height = length(unique(targets))*38, marker = list(line = list(color = 'rgba(0, 0, 0, 0.4)', width = 1)))  %>%

layout(
  title = "",
  xaxis = list(title = paste0(gene, " mutation frequency")),
  yaxis = list(title = ""),
